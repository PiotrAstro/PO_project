...
class TestClientService:
    ...
    #         description="",
    #         recipeSteps="",
    #         image_name="remove.jpg"
    #     )
    #     db.session.add(recipe)
    #     db.session.commit()

    #     # Bez powiązanych requestów -> remove powinno się udać
    #     success = remove_recipe(recipe.id, client.id)
    #     assert success is True

    #     row = db.session.execute(
    #         text('SELECT 1 FROM "Recipe" WHERE id = :r_id'),
    #         {"r_id": recipe.id}
    #     ).fetchone()
    #     assert row is None, "Recipe powinien zostać usunięty z bazy"

    #     # # Sprawdzamy, czy faktycznie zniknęło:
    #     # check = Recipe.query.get(recipe.id)
    #     # assert check is None

    #     # Drugi przypadek – recipe powiązane z Request
    #     recipe2 = Recipe(
    #         client_id=client.id,
    #         recipe_type_id=rt.id,
    #         name="BoundRecipe",
    #         description="",
    #         recipeSteps="",
    #         image_name="bound.jpg"
    #     )
    #     db.session.add(recipe2)
    #     db.session.commit()

    #     # Tworzymy Request i powiązanie w RecipeRequest
    #     req = Request(
    #         client_id=client.id,
    #         withDelivery=True,
    #         address="req address",
    ...
    #     rr = RecipeRequest(request_id=req.id, recipe_id=recipe2.id)
    #     db.session.add(rr)
    #     db.session.commit()

    #     # Teraz remove_recipe powinno zwrócić False
    #     success_2 = remove_recipe(recipe2.id, client.id)
